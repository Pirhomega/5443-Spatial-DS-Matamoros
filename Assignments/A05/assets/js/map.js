...
map.on('load', function () {
    ...
    $(document).ready(function () {
        ...
        function chooseDataset() {
            return {
                'datasets': {
                    'railroads': document.getElementById("cityRailroads").checked,
                    'primary_roads': document.getElementById("cityPrimary").checked
                }
            }
        };
        ...
        $('#cityDirect').click(function (event) {
            // get the source and destination city names from their input elements
            let citynameSource = $('#inputCitiesSource').val()
            let citynameDest = $('#inputCitiesDest').val()
            $.get("http://localhost:8888/cityDirections/?cityArgs=" + [citynameSource, citynameDest], function (distance_miles) {
                // if there is a path from source to target city, read the path and closest events from a file and display on map
                if (distance_miles != "-1") {
                    $.get("/Assignments/A05/assets/api/data/shortest_paths/" + citynameSource + "_" + citynameDest + ".geojson", function (path) {
                        $.get("/Assignments/A05/assets/api/data/shortest_paths/closest_points.geojson", function (points) {
                            cityDistFC.features = [...points.features, path]
                            loadSourceLayer()

                            // I couldn't get this stuff to work

                            // // center the view on the newly-added feature collection
                            // map.flyTo({
                            //     center: turf.center(path).geometry.coordinates
                            // });
                            // if (turf.area(turf.bboxPolygon(turf.bbox(cityDistFC))) > 1000000) {
                            //     map.fitBounds(turf.bbox(cityDistFC), { padding: 200 })
                            // }
                            ...
                            // display the path's distance in miles
                            rounded_distance = Math.round(parseInt(distance_miles) * 100) / 100;
                            lineAnswer = document.getElementById('calculated-distance');
                            lineAnswer.innerHTML = '<p>' + rounded_distance + ' mi</p>';

                            // the score is only derived from the number of events that happend within 0.2 degrees
                            //      of the path
                            lineAnswer = document.getElementById('score');
                            lineAnswer.innerHTML = '<p>' + points.features.length + ' points</p>';
                        })
                    })
                }
                else {
                    // if there is no path from source to target city, display it to webpage
                    lineAnswer = document.getElementById('score');
                    lineAnswer.innerHTML = '<p>' + "No path" + ' points</p>';
                }
            })
        })
